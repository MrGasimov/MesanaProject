...
public class GuiBuilder {
	...
	List customerList;
	Text customerText, commentText, measurTaskText, sensorTaskText;
	StyledText sensorText, measureText;
			...
			public void run() {
				while (true) {
					Display.getDefault().syncExec(new Runnable() {
						@Override
						public void run() {
							// listenes if sensor disconnected or connected
							if (!ConfigSensor.checkSensorConnected()) {
								resetData();
	...
	public void setGui() {
		...
		customerText = new Text(shell, SWT.BORDER | SWT.V_SCROLL | SWT.READ_ONLY);
		...
		// Comment Text
		commentText = new Text(shell, SWT.BORDER | SWT.V_SCROLL | SWT.READ_ONLY);
		gridData = new GridData();
		...
		measurTaskText = new Text(shell, SWT.BORDER | SWT.V_SCROLL | SWT.READ_ONLY);
		gridData = new GridData();
		gridData.horizontalAlignment = GridData.CENTER;
		gridData.verticalAlignment = GridData.BEGINNING;
		gridData.heightHint = 80;
		...
		// Sensor Task Text
		sensorTaskText = new Text(shell, SWT.BORDER | SWT.V_SCROLL | SWT.READ_ONLY);
		gridData = new GridData();
		gridData.horizontalAlignment = GridData.CENTER;
		gridData.verticalAlignment = GridData.BEGINNING;
		gridData.heightHint = 80;
			...
			public void widgetSelected(SelectionEvent e) {
				try {
					...
					if (ConfigSensor.checkConfigState()) {
						configButton.setText("Configurated!");
					} else {
						configButton.setText("Error");
					}
	...
	public void StyledMeasureText(StyledText text){
		StyleRange style1 = new StyleRange();
	    style1.start = 0;
	    style1.length = 11;
	    style1.fontStyle = SWT.BOLD;
	    text.setStyleRange(style1);
	    int i = sensorText.getLineAtOffset(22);
	...
	public void StyledSensorText(StyledText text) {
		StyleRange style1 = new StyleRange();
		style1.start = 0;
		style1.length = 11;
		style1.fontStyle = SWT.BOLD;
